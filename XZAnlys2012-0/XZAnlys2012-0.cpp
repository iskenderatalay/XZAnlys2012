// XZAnlys2012-0.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
#include <iostream>
...
int main(int argc, char* argv[])
{
    ...
    double* A, * E, * I, * Dx, * Dy, * Aci, * AciDugum, * AciDugumRad, * L, * delx, * dely, ** Sj, ** S, * P, * N, * W, * a, ** Q, * Qj, * Qt, * Pj, * X, pivot, eksiilecarp, rep1, ** qussu, ** qussucizgi, ** qcubuk, ** Djyeni;
    ofstream yaz;
    ifstream oku;
    ...
    if (das == 0)
    {
        ...
        yaz << "Acili Konumda Dugum Yok" << endl;
    }
    else
    {
        for (i = 0; i < das; i++)
        {
            ...
            yaz << "Dugum Numarasi =  " << dgmno << endl;
            yaz << "Aci Degeri = " << AciDugum[dgmno - 1] << endl << endl;
        }
    }
    cout << endl;
    cout << "Dugum Acilari Matrisimiz = " << endl;
    yaz << "Dugum Acilari Matrisimiz = " << endl;
    cout << endl;
    yaz << endl;
    for (i = 0; i < x; i++)
    ...
    for (i = 0; i < n; i++)
    {
        Q[i][0] = (cos(Aci[i] - AciDugumRad[R[i] - 1]) * N[i] * ((L[i] - a[i]) / L[i])) - (sin(Aci[i] - AciDugumRad[R[i] - 1]) * ((W[i] * L[i] / 2.0) + ((P[i] * pow((L[i] - a[i]), 2.0)) * (3.0 * a[i] + (L[i] - a[i])) / pow(L[i], 3.0))));
        Q[i][1] = (sin(Aci[i] - AciDugumRad[R[i] - 1]) * N[i] * ((L[i] - a[i]) / L[i])) + (cos(Aci[i] - AciDugumRad[R[i] - 1]) * ((W[i] * L[i] / 2.0) + ((P[i] * pow((L[i] - a[i]), 2.0)) * (3.0 * a[i] + (L[i] - a[i])) / pow(L[i], 3.0))));
        Q[i][2] = (W[i] * pow(L[i], 2.0) / 12.0) + (P[i] * a[i] * (pow((L[i] - a[i]), 2.0)) / pow(L[i], 2.0));
        Q[i][3] = (cos(Aci[i] - AciDugumRad[K[i] - 1]) * N[i] * (a[i] / L[i])) - (sin(Aci[i] - AciDugumRad[K[i] - 1]) * ((W[i] * L[i] / 2.0) + (P[i] * pow(a[i], 2.0) * (3.0 * (L[i] - a[i]) + a[i]) / pow(L[i], 3.0))));
        Q[i][4] = (sin(Aci[i] - AciDugumRad[K[i] - 1]) * N[i] * (a[i] / L[i])) + (cos(Aci[i] - AciDugumRad[K[i] - 1]) * ((W[i] * L[i] / 2.0) + (P[i] * pow(a[i], 2.0) * (3.0 * (L[i] - a[i]) + a[i]) / pow(L[i], 3.0))));
    ...
    cin >> yss;
    yaz << "Yuklu Serbestlik Sayisi = " << yss << endl << endl;
    if (yss == 0)
    {
        cout << endl;
        yaz << endl;
    ...
    cout << endl;
    cout << "Pj Matrisimiz Verilerin Kaydedildigi Cikti Dosyasindadir. " << endl << endl;
    ...
    for (i = 0; i < n; i++)
    {
        Sj[0][0] = (pow(cos(Aci[i] - AciDugumRad[R[i] - 1]), 2.0) * (E[i] * A[i]) / L[i]) + (pow(sin(Aci[i] - AciDugumRad[R[i] - 1]), 2.0) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[0][1] = (sin(Aci[i] - AciDugumRad[R[i] - 1]) * cos(Aci[i] - AciDugumRad[R[i] - 1]) * (E[i] * A[i]) / L[i]) - (sin(Aci[i] - AciDugumRad[R[i] - 1]) * cos(Aci[i] - AciDugumRad[R[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[0][2] = -(sin(Aci[i] - AciDugumRad[R[i] - 1]) * (6.0 * E[i] * I[i]) / pow(L[i], 2.0));
        Sj[0][3] = ((cos(Aci[i] - AciDugumRad[R[i] - 1]) * cos(Aci[i] - AciDugumRad[K[i] - 1])) * (-E[i] * A[i]) / L[i]) - ((sin(Aci[i] - AciDugumRad[R[i] - 1]) * sin(Aci[i] - AciDugumRad[K[i] - 1])) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[0][4] = ((cos(Aci[i] - AciDugumRad[R[i] - 1]) * sin(Aci[i] - AciDugumRad[K[i] - 1])) * (-E[i] * A[i]) / L[i]) + ((sin(Aci[i] - AciDugumRad[R[i] - 1]) * cos(Aci[i] - AciDugumRad[K[i] - 1])) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[0][5] = Sj[0][2];
        Sj[1][0] = ((cos(Aci[i] - AciDugumRad[R[i] - 1]) * sin(Aci[i] - AciDugumRad[R[i] - 1])) * (E[i] * A[i]) / L[i]) - ((sin(Aci[i] - AciDugumRad[R[i] - 1]) * cos(Aci[i] - AciDugumRad[R[i] - 1])) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[1][1] = (pow(sin(Aci[i] - AciDugumRad[R[i] - 1]), 2.0) * (E[i] * A[i]) / L[i]) + (pow(cos(Aci[i] - AciDugumRad[R[i] - 1]), 2.0) * (12.0 * E[i] * I[i]) / (pow(L[i], 3.0)));
        Sj[1][2] = (cos(Aci[i] - AciDugumRad[R[i] - 1]) * (6.0 * E[i] * I[i]) / pow(L[i], 2.0));
        Sj[1][3] = (-sin(Aci[i] - AciDugumRad[R[i] - 1]) * cos(Aci[i] - AciDugumRad[K[i] - 1]) * (E[i] * A[i]) / L[i]) + (cos(Aci[i] - AciDugumRad[R[i] - 1]) * sin(Aci[i] - AciDugumRad[K[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[1][4] = (-sin(Aci[i] - AciDugumRad[R[i] - 1]) * sin(Aci[i] - AciDugumRad[K[i] - 1]) * (E[i] * A[i]) / L[i]) - (cos(Aci[i] - AciDugumRad[R[i] - 1]) * cos(Aci[i] - AciDugumRad[K[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[1][5] = Sj[1][2];
        Sj[2][0] = Sj[0][2];
        Sj[2][1] = Sj[1][5];
        Sj[2][2] = (4.0 * E[i] * I[i] / L[i]);
        Sj[2][3] = (sin(Aci[i] - AciDugumRad[K[i] - 1]) * (6.0 * E[i] * I[i]) / pow(L[i], 2.0));
        Sj[2][4] = -(cos(Aci[i] - AciDugumRad[K[i] - 1]) * (6.0 * E[i] * I[i]) / pow(L[i], 2.0));
        Sj[2][5] = (Sj[2][2]) / 2.0;
        Sj[3][0] = (-cos(Aci[i] - AciDugumRad[K[i] - 1]) * cos(Aci[i] - AciDugumRad[R[i] - 1]) * (E[i] * A[i]) / L[i]) - (sin(Aci[i] - AciDugumRad[K[i] - 1]) * sin(Aci[i] - AciDugumRad[R[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[3][1] = (-cos(Aci[i] - AciDugumRad[K[i] - 1]) * sin(Aci[i] - AciDugumRad[R[i] - 1]) * (E[i] * A[i]) / L[i]) + (sin(Aci[i] - AciDugumRad[K[i] - 1]) * cos(Aci[i] - AciDugumRad[R[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[3][2] = (sin(Aci[i] - AciDugumRad[K[i] - 1]) * (6.0 * E[i] * I[i]) / pow(L[i], 2.0));
        Sj[3][3] = (pow(cos(Aci[i] - AciDugumRad[K[i] - 1]), 2.0) * (E[i] * A[i]) / L[i]) + (pow(sin(Aci[i] - AciDugumRad[K[i] - 1]), 2.0) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[3][4] = (cos(Aci[i] - AciDugumRad[K[i] - 1]) * sin(Aci[i] - AciDugumRad[K[i] - 1]) * (E[i] * A[i]) / L[i]) - (sin(Aci[i] - AciDugumRad[K[i] - 1]) * cos(Aci[i] - AciDugumRad[K[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[3][5] = Sj[3][2];
        Sj[4][0] = (-sin(Aci[i] - AciDugumRad[K[i] - 1]) * cos(Aci[i] - AciDugumRad[R[i] - 1]) * (E[i] * A[i]) / L[i]) + (cos(Aci[i] - AciDugumRad[K[i] - 1]) * sin(Aci[i] - AciDugumRad[R[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[4][1] = (-sin(Aci[i] - AciDugumRad[K[i] - 1]) * sin(Aci[i] - AciDugumRad[R[i] - 1]) * (E[i] * A[i]) / L[i]) - (cos(Aci[i] - AciDugumRad[K[i] - 1]) * cos(Aci[i] - AciDugumRad[R[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[4][2] = -(cos(Aci[i] - AciDugumRad[K[i] - 1]) * (6.0 * E[i] * I[i]) / pow(L[i], 2.0));
        Sj[4][3] = (sin(Aci[i] - AciDugumRad[K[i] - 1]) * cos(Aci[i] - AciDugumRad[K[i] - 1]) * (E[i] * A[i]) / L[i]) - (sin(Aci[i] - AciDugumRad[K[i] - 1]) * cos(Aci[i] - AciDugumRad[K[i] - 1]) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[4][4] = (pow(sin(Aci[i] - AciDugumRad[K[i] - 1]), 2.0) * (E[i] * A[i]) / L[i]) + (pow(cos(Aci[i] - AciDugumRad[K[i] - 1]), 2.0) * (12.0 * E[i] * I[i]) / pow(L[i], 3.0));
        Sj[4][5] = Sj[4][2];
        Sj[5][0] = Sj[0][2];
        Sj[5][1] = Sj[1][2];
        Sj[5][2] = Sj[2][5];
    ...
    for (i = 0; i < sd; i++)
        Qj[i] = 0;
    cout << "Toplanmis S Matrisi Verilerin Kaydedildigi Cikti Dosyasindadir. " << endl << endl;
    ...
    cout << "Toplanmis Qj Matrisi Verilerin Kaydedildigi Cikti Dosyasindadir. " << endl << endl;
    ...
    cout << "Sistemin Toplam Yuk Matrisi Kaydedildigi Cikti Dosyasindadir. " << endl << endl;
    ...
    cout << "Ust Ucgen Matrisimiz Verilerin Kaydedildigi Cikti Dosyasindadir. " << endl << endl;
    ...
    cout << endl;
    yaz << endl;
    cout << "Sistemin Toplam Yuk Matrisinin Islemler Sonucundaki Hali Verilerin Kaydedildigi Cikti Dosyasindadir. " << endl << endl;
    ...
    for (i = 0; i < n; i++)
    {
        qussucizgi[i][0] = N[i] * (L[i] - a[i]) / L[i];
        ...
        qussu[i][1] = ((12.0 * E[i] * I[i]) / pow(L[i], 3.0)) * ((-sin(Aci[i] - AciDugumRad[R[i] - 1]) * Djyeni[i][0]) + (cos(Aci[i] - AciDugumRad[R[i] - 1]) * Djyeni[i][1]) + (sin(Aci[i] - AciDugumRad[K[i] - 1]) * Djyeni[i][3]) - (cos(Aci[i] - AciDugumRad[K[i] - 1]) * Djyeni[i][4])) + (((6.0 * E[i] * I[i]) / pow(L[i], 2.0)) * (Djyeni[i][2] + Djyeni[i][5]));
        qussu[i][2] = ((6.0 * E[i] * I[i]) / pow(L[i], 2.0)) * ((-sin(Aci[i] - AciDugumRad[R[i] - 1]) * Djyeni[i][0]) + (cos(Aci[i] - AciDugumRad[R[i] - 1]) * Djyeni[i][1]) + (sin(Aci[i] - AciDugumRad[K[i] - 1]) * Djyeni[i][3]) - (cos(Aci[i] - AciDugumRad[K[i] - 1]) * Djyeni[i][4])) + (((4 * E[i] * I[i]) / L[i]) * Djyeni[i][2]) + (((2 * E[i] * I[i]) / L[i]) * Djyeni[i][5]);
        qussu[i][3] = -qussu[i][0];
        qussu[i][4] = -qussu[i][1];
        qussu[i][5] = ((6.0 * E[i] * I[i]) / pow(L[i], 2.0)) * ((-sin(Aci[i] - AciDugumRad[R[i] - 1]) * Djyeni[i][0]) + (cos(Aci[i] - AciDugumRad[R[i] - 1]) * Djyeni[i][1]) + (sin(Aci[i] - AciDugumRad[K[i] - 1]) * Djyeni[i][3]) - (cos(Aci[i] - AciDugumRad[K[i] - 1]) * Djyeni[i][4])) + ((4 * E[i] * I[i] / L[i]) * Djyeni[i][5]) + (((2 * E[i] * I[i]) / L[i]) * Djyeni[i][2]);
